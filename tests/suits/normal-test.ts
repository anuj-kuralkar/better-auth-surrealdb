...
export const getNormalTestSuiteTests = (
	...
	return {
		"create - should create a model": async () => {
			...
			// The adapter returns IDs in "table:id" format (RecordId-style).
			// transformGeneratedModel doesn't know about this, so we patch the
			// expected id to match the adapter's output format.
			if (!transformIdOutput && !result.id.includes(":")) {
				// plain id — no patching needed
			} else if (!transformIdOutput && result.id.includes(":")) {
				transformed.id = result.id;
			}
		...
		"create - should use generateId if provided": async () => {
			...
			// The adapter may return IDs in "table:id" format. If transformIdOutput
			// is not provided, accept the adapter's ID as long as it contains the
			// original generated ID.
			if (transformIdOutput) {
				expect(res.id).toEqual(transformIdOutput(ID));
			} else {
				expect(res.id === ID || res.id.endsWith(`:${ID}`)).toBe(true);
			}
